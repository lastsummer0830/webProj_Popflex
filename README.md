...
**문제** — 필름 다이어리는 **독립적인 기능이 아니었습니다.**
예매가 끝나야 다이어리가 생기고, 영화의 장르·감독을 알아야 뱃지를 판정하고, 리뷰를 쓰면 다이어리에 연결되고, 회원이 있어야 내 기록이 됩니다.

즉 **팀원 4명이 각자 만들고 있는 도메인 전부에 제 기능이 의존**했습니다.
`DIARY_ENTRY` 테이블이 FK를 4개 갖고 있다는 게 그 증거입니다.
...
문제는 **다들 동시에 만드는 중**이었다는 것입니다.
팀원이 예매 로직을 고치면 제 다이어리가 깨지고, 영화 테이블 컬럼이 바뀌면 뱃지 집계가 멈췄습니다.
남의 코드가 끝나기를 기다리면 제 파트는 시작조차 못 하는 상황이었습니다.
...
| 영화 정보 | 다이어리가 직접 조회하지 않고 **`MOVIE_ID`로 JOIN**해서 필요한 순간에만 읽는다 |

그리고 **DB 설계를 제가 맡고 있었기 때문에**, 이 접점들을 스키마 레벨에서 먼저 못 박았습니다.
`ON DELETE SET NULL`을 걸어 **리뷰나 예매가 지워져도 다이어리는 살아남게** 한 것도 이때 결정한 것입니다 — 기록은 남의 사정으로 사라지면 안 되니까요.

**결과** — 팀원 파트가 아직 미완성이어도 **다이어리를 먼저 만들 수 있었습니다.**
ID만 있으면 되니까, 예매 화면이 완성되기 전에도 다이어리는 테스트할 수 있었습니다.

**배운 것** — 여러 사람이 동시에 만드는 프로젝트에서는 **"무엇을 아느냐"보다 "무엇을 모르는 채로 둘 수 있느냐"** 가 중요하다는 걸 배웠습니다.
남의 테이블 구조를 알아야만 돌아가는 코드를 짰다면, 그쪽이 바뀔 때마다 제 코드가 따라 바뀌었을 겁니다.

---

<!-- ⚠ 미확인(조아진 확답 대기 2026-07-14): 아래 "시도한 방법" 표의 ①②는 Claude가 추정으로 채운 것.
     - MEMBER_BADGE 라는 테이블명이 실제 이름이 맞는지
     - 트리거 방식을 실제로 검토했는지
     확답 전까지 push 금지. 사실이 아니면 해당 줄 삭제할 것. -->
...
<!-- ⚠ 미확인(조아진 확답 대기 2026-07-14): "처음엔 TO_CHAR(...,'YYYY-WW')를 썼다"가 실제인지 확인 필요.
     (문제 자체 = 연말·연초 누락은 이력서 근거 있음. 최종 해결 IW는 코드로 확인됨.) -->
